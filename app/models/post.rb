class Post < ApplicationRecord
  ...
  def add_error
    ...
    # 魚種が空のときにエラーメッセージを追加する
    if name.blank?
      errors[:base] << "魚種が入力されていません"
    end
  ...
  def self.search_for(content, method)
    ...
    @state_array.each_with_index do |state, index|
      if state.match(".*" + content + ".*")
        state_num = index
      end
    end
    where_string = "name like '%" + content + "%' or address like '%" + content + "%' "
    if state_num != nil
      where_string += "or state = " + state_num.to_s
    end
    Post.where(where_string)
  end
end
